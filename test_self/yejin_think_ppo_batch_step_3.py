...
def train_ppo_stable(env, model, optimizer, num_total_steps=50000,
                     ...
                     eps_clip=1.0, K_epochs=4, batch_size=64,
    ...
    # ✨ [개선] 최근 100개 에피소드의 성공 여부를 추적 (1: 성공, 0: 실패/타임아웃)
    ...
    for global_step in range(0, num_total_steps, rollout_length):
        ...
        for step in range(rollout_length):
            ...
            if done:
                ...
                # ✨ [개선] info를 바탕으로 종료 이유와 성공 여부를 결정
                reason = info.get('termination_reason', 'unknown')
                if reason == 'success':
                    termination_log = "성공 (Success)"
                    recent_successes.append(1)
                elif reason == 'timeout':
                    termination_log = "타임아웃 (Timeout)"
                    recent_successes.append(0)
                else:
                    termination_log = f"기타 ({reason})"
                    recent_successes.append(0)

                # ✨ [개선] 에피소드 종료 로그에 '결과' 항목 추가
                print(
...
if __name__ == '__main__':
    env = GridEnv(grid_rows=4, grid_cols=5, max_steps=700)
